...
### Función strlen, longitud de cadena
...
Inspección de la firma:
```c
// Esta será una sintaxis muy similar entre todas las funciones de strings.
// ya no es necesario incluir el tamaño del arreglo, porque sabemos que
// todos los strings terminan con el caracter nulo.
// 
// regresa el tamaño.
// size_t es un alias para un tipo de dato
// unsigned int de al menos 16 bits
// |
// |        es const porque no se modificará
// |        la referencia al string
// |        dentro de la funcion.
// |            |
// |            |    se tiene una sintaxis distinta para 
// |            |    indicar que es un arreglo de caracteres.
// |            |    Es lo mismo que char str[].
// |            |    En específico, es un apuntador a char.
// |            |      |
// v            v      v
size_t strlen(const char* str);
//      ^                  ^
//      |                  |
//  strlen,           str es el nombre
//  corto para        local a la función
//  string length     del arreglo de caracteres
// 
// 
```

Y esta es la forma de utilizarla:
...
### Función strcpy, copiar un string en otro
Si queremos copiar un string en otro, no podemos hacerlo directamente con el operador de asignación. Necesitamos una función que nos ayude con esto, llamada `strcpy`:
...
int main() {
    char nombre1[] = "Carlos";
    char nombre2[100]; // Necesitamos un arreglo lo suficientemente grande para guardar el string.

    strcpy(nombre2, nombre1);
    printf("El nombre copiado es: %s\n", nombre2);
    return 0;
}
```
